...
def print_model_prediction_result(model, activities, resources, amount ):
    out, _ = model(activities, input_resources = resources, amount = amount)
    predicted_vocab_distributions = tf.gather(out, activities.shape[-1]-1, axis=1)
    predicted_vocab_distributions_df = pd.DataFrame(predicted_vocab_distributions.numpy().tolist(),columns=model.vocab.vocabs)
    max_arg = tf.math.argmax(predicted_vocab_distributions, axis=-1).numpy()[0]
    max_prob_vocab = model.vocab.index_to_vocab(max_arg)
    print_big("Predicted activity with highest probability (%.2f) is \"%s\"" % (predicted_vocab_distributions[0][max_arg].numpy(), max_prob_vocab), "Predict result", num_marks=40)
    print("\n\n")
